  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-stone-100 last:border-0">
      ...
      <div className="text-right flex-shrink-0">
        <p className={`text-sm font-semibold ${isIncoming ? "text-emerald-600" : "text-gray-800"}`}>
          {isIncoming ? "+" : "-"}{formatAmount(tx.amount)}
        </p>
        <span className={`inline-block mt-1 px-2 py-0.5 text-xs rounded-full border ${statusStyle[tx.status]}`}>
...
export default function DashboardPage() {
  ...
  const { data: transactions, isLoading: loadingTx } = useTransactions(firstAccountId);
  ...
  const savingsBalance =
    accounts?.filter((a) => a.type === "savings").reduce((sum, a) => sum + a.balance, 0) ?? 0;
  ...
  return (
    <div className="p-8 max-w-5xl">
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
              <span className="text-emerald-700 font-semibold text-xs">MAD</span>
            </div>
            <div>
              <p className="text-xl font-bold text-emerald-700">{formatAmount(totalBalance)}</p>
              <p className="text-xs text-gray-400 uppercase tracking-wide mt-0.5">Total Balance</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-bold text-gray-900">{formatAmount(savingsBalance)}</p>
              <p className="text-xs text-gray-400 uppercase tracking-wide mt-0.5">Savings</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        <h2 className="text-base font-semibold text-gray-900 mb-4">My Accounts</h2>
